module.exports = (App) => {
  ...
  App.add_admin = (ox) => {
    ...
    if (!App.db.permissions.admins.includes(ox.arg)) {
      ...
      App.save_file(`permissions.json`, App.db.permissions, (err) => {
        App.process_feedback(ox.ctx, ox.data, `Username "${ox.arg}" was successfully added as an admin.`, true)
      })
    }
  }
  ...
  App.remove_admin = (ox) => {
    ...
    if (App.db.permissions.admins.includes(ox.arg)) {
      ...
      App.save_file(`permissions.json`, App.db.permissions, (err) => {
        App.process_feedback(ox.ctx, ox.data, `Username "${ox.arg}" was successfully removed as an admin.`, true)
      })
    }
    else {
  ...
  App.clear_admins = (ox) => {
    ...
    App.save_file(`permissions.json`, App.db.permissions, () => {
      App.process_feedback(ox.ctx, ox.data, `Admins list successfully cleared.`, true)
    })
  }
}